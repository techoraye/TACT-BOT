  ...
  if (userData.guilds) {
    userData.guilds.forEach((guild) => {
      ...
      guild.settingsUrl = client.guilds.cache.get(guild.id)
        ? `/manage/${guild.id}/`
        : (client.user && client.user.id ? `https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=2146958847&guild_id=${guild.id}&redirect_uri=http%3A%2F%2Flocalhost:20125%2Fselector` : '#');
