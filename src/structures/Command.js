...
/**
 * @typedef {"ADMIN"|"AUTOMOD"|"ECONOMY"|"FUN"|"IMAGE"|"INFORMATION"|"INVITE"|"MODERATION"|"ERELA_JS"|"NONE"|"OWNER"|"SOCIAL"|"SUGGESTION"|"TICKET"|"UTILITY"} CommandCategory
 */
...
/**
 * Handle the execution of slash commands
 */
async function handleSlashCommand(interaction) {
  const command = interaction.client.slashCommands.get(interaction.commandName);
  if (!command) {
    return interaction.reply({
      content: "That slash command does not exist.",
      ephemeral: true,
    });
  }

  try {
    // ✅ Run the interactionRun defined in the command file
    await command.interactionRun(interaction, {});
  } catch (err) {
    interaction.client.logger.error(`❌ Slash command error: ${interaction.commandName}`, err);
